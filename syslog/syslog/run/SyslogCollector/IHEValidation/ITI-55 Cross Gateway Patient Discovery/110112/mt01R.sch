...
<schema xmlns="http://purl.oclc.org/dsdl/schematron">
    <title>Schematron rules for ITI-55/110112 ITI TF2-b R11 3.55.5.1.1</title>
    <pattern>
        ...
        <rule id="EventIdentification" context="/AuditMessage/EventIdentification">
            ...
            <assert test="EventTypeCode[@code='ITI-55']"
                >EventTypeCode@code must be 'ITI-55'</assert>
        ...
        <rule id="top level elements" context="/AuditMessage">
            ...
            <assert id="PatientElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='1' and @ParticipantObjectTypeCodeRole='1'])=0"
                >Must not contain Patient ParticipantObjectIdentification (TypeCode = 1 and TypeCodeRole = 1)</assert>
            
            <assert id="QueryParametersElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='24'])=1"
        ...
        <rule id="Query parameters" context="/AuditMessage/ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='24']">
        		<assert test="ParticipantObjectIDTypeCode[@code='ITI-55']">Query ParticipantObjectIDTypeCode code must be 'ITI-55'</assert>
        		...
        		<assert test="@ParticipantObjectID!=''">Query must have ParticipantObjectID</assert>
        		<assert test="ParticipantObjectQuery!=''">Query must have ParticipantObjectQuery QueryByParameter segment (base64)</assert>
        </rule>
    </pattern>
</schema>