    ...
    <pattern>
        ...
        <rule id="EventIdentification" context="/AuditMessage/EventIdentification">
            ...
            <assert test="@EventActionCode='C' or @EventActionCode='U'"
                >EventActionCode must be 'C' for A01, A04, A05 or 'U' for A08</assert>
        ...
        <rule id="TopLevelElements" context="/AuditMessage">
            ...
            <assert id="ValueSetElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and 
                                @ParticipantObjectTypeCodeRole='3'])=1"
                >Must contain one Value Set Instance ParticipantObjectIdentification (TypeCode = 2 and TypeCodeRole = 3)</assert>
        ...
        <rule id="Query parameters" context="/AuditMessage/ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='24']">
        		...
        		<assert test="ParticipantObjectIDTypeCode[@code='ITI-18']">Query ParticipantObjectIDTypeCode code must be 'ITI-18'</assert>
        ...
        <rule id="Document" context="/AuditMessage/ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='3']">
        		...
        		<assert test="count(ParticipantObjectDetail[@type='Repository Unique Id' and @value!=''])=1"
        			>Query must have 'Repository Unique Id' detail element</assert>
        		<assert test="count(ParticipantObjectDetail[@type='ihe:homeCommunityID' and @value!=''])=1"
        			>Query must have 'homeCommunityID' detail element</assert>
        </rule>
        ...
        <rule id="ValueSetInstance" context="/AuditMessage/ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='3']">
        		<assert test="ParticipantObjectIDTypeCode[@code='9']">ValueSetInstance ParticipantObjectIDTypeCode code must be '9'</assert>
        		...
        		<assert test="count(ParticipantObjectDetail[@type='Value Set Version' and @value!=''])=1"
        			>Value set instance must have 'Value Set Version' detail element</assert>
        </rule>
