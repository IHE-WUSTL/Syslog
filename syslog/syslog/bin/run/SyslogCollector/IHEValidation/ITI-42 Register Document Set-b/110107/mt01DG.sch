<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron">
    <title>Schematron rules for ITI-42/110107 Ref: ITI TF-2b 3.42.7.1.2</title>
    <pattern>
        ...
        <rule id="EventIdentification" context="/AuditMessage/EventIdentification">
            ...
            <assert test="EventTypeCode[@csd-code='ITI-42']">EventTypeCode@code must  be 'ITI-42'</assert>
        ...
        <rule id="topLevelElements" context="/AuditMessage">
            ...
            <assert d="DestinationElement" test="count(ActiveParticipant/RoleIDCode[@csd-code='110152'])=1">Must contain one Destination ActiveParticipant (RoleIDCode = 110152)</assert>

            <assert d="AuditSourceElement" test="count(AuditSourceIdentification)=1">Must contain one AuditSourceIdentification</assert>
            
            <assert d="PatientElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='1' and                                  @ParticipantObjectTypeCodeRole='1'])=1">Must contain one Patient ParticipantObjectIdentification (TypeCode = 1 and TypeCodeRole = 1)</assert>
        ...
        <rule id="Destination" context="/AuditMessage/ActiveParticipant[RoleIDCode[@csd-code='110152']]">
        		<assert test="@UserID!=''">Destination must have UserID attribute</assert>
        		<assert test="@AlternativeUserID!=''">Destination must have AlternativeUserID attribute</assert>
