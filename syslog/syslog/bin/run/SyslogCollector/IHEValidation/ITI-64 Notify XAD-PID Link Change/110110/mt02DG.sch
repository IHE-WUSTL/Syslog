<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron">
    <title>Schematron rules for ITI-64/110110 Ref: ITI_Suppl_XPID_Rev1-1_TI_2011-08_19 
    ...
    <pattern>
        ...
        <rule id="EventIdentification" context="/AuditMessage/EventIdentification">
            ...
            <assert test="@EventActionCode='U'">EventActionCode must be U' (Update) for A43</assert>
        
            <assert test="EventTypeCode[@csd-code='ITI-64']">EventTypeCode@code must  be 'ITI-64'</assert>
        ...
        <rule id="TopLevelElements" context="/AuditMessage">
            ...
            <assert id="PatientElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='1' and                                  @ParticipantObjectTypeCodeRole='1'])=3">Must contain one Patient ParticipantObjectIdentification (TypeCode = 1 and TypeCodeRole = 1)</assert>
