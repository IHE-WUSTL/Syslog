<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron">
    <title>Schematron rules for ITI-36/110106 IHE_QRPH_Supl_CRD 2012-9-24 5.Z3.3.1</title>
    <pattern>
        ...
        <rule id="EventIdentification" context="/AuditMessage/EventIdentification">
            ...
            <assert test="EventTypeCode[@csd-code='ITI-36']">EventTypeCode@code must be 'ITI-36'</assert>
        ...
        <rule id="top level elements" context="/AuditMessage">
            ...
            <assert id="SubjectElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='1' and                                  @ParticipantObjectTypeCodeRole='1'])&lt;=1">Must contain one Subject ParticipantObjectIdentification (TypeCode = 1 and TypeCodeRole = 1)</assert>
            
            <assert id="FormDataElement" test="count(ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and                                  @ParticipantObjectTypeCodeRole='20'])=1">Must contain one Query Form Data ParticipantObjectIdentification (TypeCode = 2 and TypeCodeRole = 20)</assert>
        ...
        <rule id="Form Data" context="/AuditMessage/ParticipantObjectIdentification[@ParticipantObjectTypeCode='2' and @ParticipantObjectTypeCodeRole='20']">
        		<assert test="ParticipantObjectIDTypeCode[@csd-code='2']">Query ParticipantObjectIDTypeCode code must be '2'</assert>
        		...
        		<assert test="@ParticipantObjectID!=''">Query must have ParticipantObjectID (Form unique ID</assert>
        </rule>
    </pattern>
</schema>